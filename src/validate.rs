...
pub trait Validate<T: Clone + Debug> {
    fn validate<Predicate: FnOnce(&T) -> std::result::Result<(), String>>(self, predicate: Predicate) -> ValidateProxy<T>;
}
...
#[cfg(test)]
mod tests {
    use crate::node::{Cherries, Leaf, Cherry};
    ...
    fn it_works() {
        ...
        let y = Leaf::new()
            ...
            .build();
        let res = x * y;
        let validated = res.validate(|quantity| {
